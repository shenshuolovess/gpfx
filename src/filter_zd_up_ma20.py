...
OUTPUT_PREFIX = "震荡上行_上升_赶顶_20日均线附近"
LOG_PREFIX = "筛选日志"


def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="筛选强趋势且位于 20 日均线附近的股票")
...
def main(argv=None):
    ...
    logger = get_rotating_logger("stock_filter", log_file)
    ...
    logger.info(f"筛选条件2：文件2 {MA20_COL} 在 [{MA20_MIN}, {MA20_MAX}] 之间")
    ...
    logger.info(f"文件2关键列检查：股票代码、名称、{MA20_COL}")
    logger.info(f"文件3模板列名：{list(df3.columns)}")

    require_columns(df1, ["代码", "名称", "市场", "分类"], "文件1")
    require_columns(df2, ["股票代码", "名称", MA20_COL], "文件2")
    ...
    logger.info(f"步骤1去重前代码数：{step1['_标准代码'].nunique()}")

    # 额外打印三类分别命中多少，方便你排查
    class_counts = step1["_分类清洗"].value_counts().to_dict()
    logger.info(f"步骤1分类分布：{class_counts}")

    df2[MA20_COL] = pd.to_numeric(df2[MA20_COL], errors="coerce")
    ma20_missing = df2[MA20_COL].isna().sum()
    if ma20_missing > 0:
        logger.warning(f"文件2中【{MA20_COL}】无法转为数值或为空：{ma20_missing} 行，这些行不会通过均线偏差筛选")
    ...
    step2 = df2_in_step1[
        df2_in_step1[MA20_COL].between(MA20_MIN, MA20_MAX, inclusive="both")
    ].copy()

    logger.info(f"步骤2完成：同时满足20日均线偏差区间的股票数量：{len(step2)}，去重代码数：{step2['_标准代码'].nunique()}")
    ...
    if len(out) > 0:
        ...
        logger.warning("最终结果为空，请检查：1）分类名称是否完全为震荡上行/上升/赶顶；2）文件2代码格式是否一致；3）20日均线偏差是否为小数口径")
