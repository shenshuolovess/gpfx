...
# 盘中监控：不启动浏览器；使用实时价量和本地历史即时重算分类，只运行一轮
.\.venv\Scripts\python.exe '.\src\推推.py' --once
```

`推推.py` 先从 `沪深.csv` 取得股票范围，再通过新浪批量行情按涨幅初筛。候选股票会把
当日实时快照追加到 `data/history` 的本地历史K线中，并直接复用主评级入口的指标和分类规则；
它不读取分类总表，也不访问东方财富页面。盘中量比使用当前累计成交量与最近5个完整交易日
成交量估算，午休时间不计入交易分钟；总市值使用实时价格和股票池中的总股本计算。

工作台中的“推推持续监控”默认每300秒运行一轮。启动后同一按钮会切换为“终止监控”，
并且后台会拒绝重复启动第二个监控进程。同一天的结果累计写入 `推推_YYYYMMDD.csv`：
已有股票保持原行不变，本轮首次出现的股票按筛选顺序追加到文件末尾，空轮次不会清空文件。
命令行使用 `--once` 时仍然只执行一轮。
...
机会评分回测生成全量明细后，可以比较两个固定的极简假设：

```powershell
.\.venv\Scripts\python.exe '.\src\validate_opportunity_factors.py' `
  --horizons 5,20,60 --train-months 12 --min-train-snapshots 24
```

- “趋势延续”只使用趋势、相对强弱和突破三个截面排名。
- “回撤反弹”只使用相对弱势、低位置和筑底三个截面排名。
- 每月月初根据过去12个月选择模型；预测周期的两倍自然日作为保守禁运期，防止使用当时
  尚未实现的未来收益。
- 过去窗口平均IC不为正或正向截面比例不足55%时，模型主动输出“不启用”。

输出包括因子诊断、逐月选择记录和滚动汇总。该验证仍使用当前股票池，而且候选假设是在
观察旧模型失败后提出，因此只能标记为“研究候选”，不能视为独立样本的实盘证明。
